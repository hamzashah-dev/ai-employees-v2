/**
 * The shape a stored schedule really has.
 *
 * `HermesCronJob.schedule` is typed `unknown` because the dashboard hands back
 * the raw job record from `cron/jobs.py`, where a stored schedule is an object —
 * `{kind: "cron", expr, display}`, `{kind: "interval", minutes, display}` or
 * `{kind: "once", run_at, display}`. Narrowed here rather than trusting a wire
 * type and printing "[object Object]".
 */
 ...
 *
 * Lives in `core` rather than in the panel because two features read it: the
 * routine rows in the employee drawer and the cross-employee routines table.
 * A second copy would be a second chance for the two to disagree about what a
 * schedule says.
 */
 ...
 *
 * Exported because the cadence control has to narrow the same `unknown` before
 * it can decide which tab an existing routine was made on — two copies of this
 * would be two chances to disagree about what a schedule says.
 */
